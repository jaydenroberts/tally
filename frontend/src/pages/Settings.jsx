...
// ─── Data tab (RM-04) ────────────────────────────────────────────────────────
// Every download here is generated by Tally and handed straight to the browser.
// Nothing is uploaded anywhere and no third party is contacted.

// The API is authenticated with a Bearer header, so a plain <a href> cannot
// fetch these files. Pull the bytes with the configured axios client, then hand
// the browser a temporary object URL to save.
...
// Errors on a blob request arrive as a Blob too, so the message has to be read
// back out before it can be shown.
async function readErrorDetail(err, fallback) {
...
const DOWNLOADS = [
  {
    ...
    body: 'A complete copy of the Tally database as a single .db file. This is the file you restore from — see the Backup & Restore guide in the documentation.',
    action: 'Download backup (.db)',
    path: '/data/backup',
    filename: 'tally-backup.db',
    variant: 'primary',
  },
  {
    id: 'json',
    title: 'Export all data (JSON)',
    body: 'Every table as one JSON file, for reading or transforming outside Tally. Password hashes are left out — use the backup above to restore an install.',
    action: 'Export all data (.json)',
    path: '/data/export',
  ...
  },
  {
    ...
    body: 'Every transaction as a spreadsheet-friendly CSV — one row per transaction, with its category and account.',
    action: 'Export transactions (.csv)',
    path: '/transactions/export?format=csv',
    filename: 'tally-transactions.csv',
    variant: 'secondary',
  },
]
...
function DataTab() {
  ...
  async function handleDownload(item) {
    ...
    try {
      ...
      setError(await readErrorDetail(err, `Could not prepare the ${item.title.toLowerCase()}.`))
    } finally {
      setBusy('')
    }
  }
  ...
  return (
    <div style={{ maxWidth: 640 }}>
      <div style={styles.card}>
        <p style={styles.sectionTitle}>Your Data</p>
        <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 16 }}>
          Take a copy of everything Tally holds, at any time. Files are generated on your
          server and downloaded straight to this device — nothing is uploaded anywhere.
        </p>
        ...
        {DOWNLOADS.map((item, index) => (
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              >
                {busy === item.id ? 'Preparing…' : item.action}
              </Button>
              {done === item.id && (
                <span style={{ color: 'var(--positive)', fontSize: 13 }}>Downloaded ✓</span>
              )}
            </div>
          </div>
        ))}
      ...
      <div style={{ ...styles.card, marginTop: 16 }}>
        <p style={styles.sectionTitle}>Automatic Backups</p>
        <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 0 }}>
          Before Tally applies any database change on upgrade, it saves a snapshot to
          the <code>backups</code> folder inside your data volume, named for the version
          being upgraded from. If that snapshot cannot be written and verified, Tally
          refuses to start rather than upgrading without a way back. The five most recent
          snapshots are kept.
        </p>
      </div>
    </div>
